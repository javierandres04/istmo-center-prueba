from books.serializers.book_serializers import ListBookLoanSerializer
...
class ListCreateLoans(APIView):
    ...
    def get(self, request):
        ...
        serializer = ListBookLoanSerializer(result_page, many=True)
    ...
    def post(self, request):
        data = request.data
        user = request.user
        book_loan = loanBookService.loan_book(data, user.id)
        serializer = ListBookLoanSerializer(book_loan)
        if book_loan:
...
class ReturnBook(APIView):
    ...
    def post(self, request):
        data = request.data
        user = request.user
        book_loan = loanBookService.return_book(data, user.id)
        serializer = ListBookLoanSerializer(book_loan)
        if book_loan:
